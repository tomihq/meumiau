...
summary: '¿Tu aplicación serverless satura tu base de datos y no sabes por qué? Este post te explica el problema del Cold Start y la creación excesiva de conexiones en entornos como AWS Lambda. Descubre cómo una correcta implementación del pooling y el patrón singleton es la solución definitiva para optimizar el rendimiento y evitar que tu base de datos colapse.'
---
## Motivación
...
1. **Cold Start** (Arranque en frío): Cuando la función se invoca por primera vez (o después de un tiempo de actividad, normalmente 5-15 minutos), el cloud provider necesita **crear un nuevo contenedor** para ejecutar el código. Este proceso es lento, ya que implica cargar todo, inicializar el runtime, etc.
2. **Warm Start** (Arranque en caliente): Si una segunda invocación llega poco después de la primera (sin haber pasado 5-15 minutos), el cloud provider **reutiliza el contenedor** que ya está listo. Esto es mucho más rápido porque el entorno ya está inicializado.
...
- `MongoServerSelectionError: "server selection error: connect ECONNREFUSED"` (La base de datos rechaza la conexión porque está saturada).
- `"Topology was not created"` (Un error genérico que a menudo indica problemas de conectividad o de saturación del pool de conexiones).
- `"MongoError: pool is exhausted"` (El pool de conexiones no tiene más conexiones disponibles).

**Para Redis:**

- `"Error: Max number of clients reached"` (Redis ha alcanzado su límite máximo de clientes conectados).
- `"OOM command not allowed when used memory > 'maxmemory'"` (Redis se queda sin memoria, a menudo exacerbado por un número excesivo de clientes y operaciones simultáneas).
- `"RedisError: Connection timeout"` (El cliente no puede establecer una conexión dentro del tiempo de espera permitido).
...
Este problema **no suele ocurrir con el modelo tradicional de VPS**, ya que en ese caso se levanta una única instancia del backend para gestionar todas las peticiones. Todos los clientes que se conectan a ese backend comparten el mismo pool de conexiones a la base de datos, lo que evita la saturación.
