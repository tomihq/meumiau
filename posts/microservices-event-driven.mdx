...
- **Cuándo tiene sentido pasar de un monolito a microservicios** y qué problemas aparecen cuando lo hacés.
- **Cómo se comunican los microservicios** (Request-Response vs Event-Driven) y cuándo elegir cada modelo.
- **Qué problemas reales resuelve una arquitectura Event-Driven** y en qué escenarios es una mala idea.
- **Cómo escalar un sistema de microservicios** usando API Gateway, Load Balancers y Service Discovery.
- **Por qué la consistencia es un problema distribuido** y cómo abordarla sin transacciones globales.
- **Cómo funciona el patrón Saga**, cuándo usarlo y qué tradeoffs introduce.
- **Cómo evolucionar APIs sin romper consumidores**, usando versionado y backward compatibility.
- **Por qué la observabilidad es obligatoria** en microservicios y cómo entender qué está pasando cuando algo falla.

Este no es un post de definiciones, sino un recorrido por **problemas reales y soluciones prácticas** que aparecen cuando un sistema crece.
